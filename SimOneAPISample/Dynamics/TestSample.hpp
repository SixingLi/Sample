...
public:
	...
	bool DriveToSpeed(const int mainVehicleId, float targetSpeed, float throttle, std::string path)
	{
		...
		SimOne_Data_Vehicle_State simOneState = SO_TimePassed;
		...
		while (1) {
			...
			SimOneAPI::RegisterSimOneVehicleState(&simOneState, 1);
			SimOneAPI::GetSimOneVehicleState(pVehExtraState.get());
