...
namespace HorizonMapEnv {
	...
	typedef struct ParkingSpaceRestriction {
		int number_limit;// 车位数量
		SSD::SimVector<NDM_TimeLimit> time_limits;// 时间限制
		SSD::SimVector<NDM_VehicleType> vehicle_types;// 停放车型
	...
	typedef struct ParkingSpace {
		SSD::SimString id;
		// 向上索引, 存储其在拓扑层所属的parking(停车场) id
		// optional string global_id = 2;  // 暂时不需要
		// 一个停车区包括一个或多个停车位
		SSD::SimStringVector parkingslot_ids;
		SSD::SimStringVector border_ids;  // 边界线id, GeneralLine
		NDM_Polygon bounding_polygon;
		SSD::SimStringVector link_ids;  ///ParkingSpace, Section, Junction
		// restriction of the parking space, 见base.proto
	...
	typedef enum SpecialSituationType {
		...
		SpecialSituationType_TollBooth = 250,         // 收费站
		SpecialSituationType_RailroadCrossing = 251,
		SpecialSituationType_PedestrianCrossing = 252,
		SpecialSituationType_SpeedBump = 253,
		SpecialSituationType_CertifiedRoad = 254,     // 检查站
		SpecialSituationType_TollBooth_CertifiedRoad = 255,  // 收费站和检查站
	...
	typedef struct Section {
		...
		NDM_SpecialSituationType special_situation_type;// section特殊情况类型，如收费站、检查站等
	}NDM_Section;
	...
	typedef struct Junction {
		...
		/// 每一条Lane用来连接路口内的可通行的两个车道(从Pred_Lane(id存储在virtual lane的pred_ids中)到Succ_Lane(id存储在succ_ids中))
		/// 包括实际存在的lane(比如路口内左转待转区)和生成的virtual lane
		SSD::SimStringVector lane_ids;
		NDM_Polygon bounding_polygon;
	...
	public:
		...
		void GetLane_(SSD::SimVector<NDM_Lane> &lanes, const SSD::SimString &laneName, const LaneSample_ &laneSample)
		{
			...
			lane.type = int(HDMapStandalone::MHDMap::GetLaneType(laneName));//这里需要对下枚举类型。
#else
			...
			lane.type = (int)laneType;//这里需要对下枚举类型。
#endif
