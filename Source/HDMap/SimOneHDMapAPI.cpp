...
#define MAX_DRIVER_NAME_LEN 10
	...
	//add v3
	SIMONE_NET_API SSD::SimVector<int> SimOneAPI::GetLaneIndexList(const SSD::SimPoint3D& pos, int& currentLaneIndex, SSD::SimStringVector& laneIdList)
	{
		return SimOneAPIService::GetLaneIndexList(pos, currentLaneIndex, laneIdList);
	}

	SIMONE_NET_API SimOneAPI::EDirectionType_ SimOneAPI::GetIconType(const SSD::SimPoint3D& pos)
	{
		return SimOneAPIService::GetIconType(pos);
	}

	SIMONE_NET_API bool SimOneAPI::GetLaneSampleByLocation(const SSD::SimPoint3D& pos, HDMapStandalone::MLaneInfo& info)
	{
