...
void ros_trans_node::pub_gps_cb(SimOne_Data_Gps *pGps)
{
  msg_gen::gps gps_d;
...
void ros_trans_node::pub_ground_truth_cb(SimOne_Data_Obstacle *pObstacle)
{
...
void ros_trans_node::pub_image_cb(int mainVehicleId, int sensorId, SimOne_Data_Image *pImage)
{
...
void ros_trans_node::pub_point_cloud_cb(int mainVehicleId, int sensorId, SimOne_Data_Point_Cloud *pPointcloud)
{
...
void ros_trans_node::pub_radar_detections_cb(int mainVehicleId, int sensorId, SimOne_Data_RadarDetection *pDetections)
{
...
void ros_trans_node::pub_sensor_detections_cb(int mainVehicleId, int sensorId, SimOne_Data_SensorDetections *pGroundtruth)
{
...
void ros_trans_node::pub_sensor_laneInfo_cb(int mainVehicleId, int sensorId, SimOne_Data_LaneInfo *pLaneInfo)
{
...
void ros_trans_node::rcv_ctl_cb(const msg_gen::control& msg)
{
  ...
  case msg.gear.EGearMode_Neutral:
    gear = EGearMode::EGearMode_Neutral;
    break;
  case msg.gear.EGearMode_Drive:
    gear = EGearMode::EGearMode_Drive;
    break;
  case msg.gear.EGearMode_Reverse:
    gear = EGearMode::EGearMode_Reverse;
    break;
  case msg.gear.EGearMode_Parking:
    gear = EGearMode::EGearMode_Parking;
    break;
  case msg.gear.EGearManualMode_1:
    gear = EGearMode::EGearManualMode_1;
    break;
  case msg.gear.EGearManualMode_2:
    gear = EGearMode::EGearManualMode_2;
    break;
  case msg.gear.EGearManualMode_3:
    gear = EGearMode::EGearManualMode_3;
    break;
  case msg.gear.EGearManualMode_4:
    gear = EGearMode::EGearManualMode_4;
    break;
  case msg.gear.EGearManualMode_5:
    gear = EGearMode::EGearManualMode_5;
    break;
  case msg.gear.EGearManualMode_6:
    gear = EGearMode::EGearManualMode_6;
    break;
  case msg.gear.EGearManualMode_7:
    gear = EGearMode::EGearManualMode_7;
    break;
  case msg.gear.EGearManualMode_8:
    gear = EGearMode::EGearManualMode_8;
    break;
  default:
    gear = EGearMode::EGearMode_Neutral;
  }
  ...
  if (!SimOneSM::SetDrive(0, pCtrl.get()))
  {
...
void ros_trans_node::rcv_pose_ctl_cb(const msg_gen::pose_control& msg)
{
    ...
    if (!SimOneSM::SetPose(0, &pose_ctl))
    {
...
{
    // ------------  simone api config start ------------
    // IP of the machine on which the SimOne simulator runs.
    SimOneAPI::SetServerInfo(bio_ip.c_str());
    ...
    while (true)
    {
        // Subscribe the main-vehicle-node with id 0, the scope of main-vehicle-id: 0-9
        if (!SimOneAPI::SubMainVehicle(0, false))
        {
            printf("\033[1m\033[31mSimOneAPI::SubMainVehicle Failed!\033[0m\n");
    ...
    while (true)
    {
        // The prerequesite of getting api work is that the value of main-vehicle-status becomes greater than 0,
        // which means the environment of running SimOne simulator is ready
        if (!SimOneAPI::GetMainVehicleStatus(&mainVehicleStatus))
        {
            printf("\033[1m\033[31mSimOneAPI::GetMainVehicleStatus Failed!\033[0m\n");
        }
        if (mainVehicleStatus.mainVehicleId == 0 && mainVehicleStatus.mainVehicleStatus > 0)
        {
            printf("SimOne is ready!\n");
            break;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    }
    ...
    SimOneAPI::SetSensorOSIDataEnable(false);
...
void ros_trans_node::run_pub()
{
  ...
  if (!SimOneAPI::SetSimOneGpsCB(pub_gps_cb)){printf("\033[1m\033[31m[run_pub]: SetSimOneGpsCB Failed!\033[0m\n");}
  if (!SimOneAPI::SetSimOneGroundTruthCB(pub_ground_truth_cb)){printf("\033[1m\033[31m[run_pub]: SetSimOneGroundTruthCB Failed!\033[0m\n");}
  if (!SimOneSM::SetImageUpdateCB(pub_image_cb)){printf("\033[1m\033[31m[run_pub]: SetImageUpdateCB Failed!\033[0m\n");}
  if (!SimOneSM::SetPointCloudUpdateCB(pub_point_cloud_cb)){printf("\033[1m\033[31m[run_pub]: SetPointCloudUpdateCB Failed!\033[0m\n");}
  if (!SimOneSM::SetRadarDetectionsUpdateCB(pub_radar_detections_cb)){printf("\033[1m\033[31m[run_pub]: SetRadarDetectionsUpdateCB Failed!\033[0m\n");}
  if (!SimOneSM::SetSensorDetectionsUpdateCB(pub_sensor_detections_cb)){printf("\033[1m\033[31m[run_pub]: SetSensorDetectionsUpdateCB Failed!\033[0m\n");}
  if (!SimOneSM::SetSensorLaneInfoCB(pub_sensor_laneInfo_cb)){printf("\033[1m\033[31m[run_pub]: SetSensorLaneInfoCB Failed!\033[0m\n");}
}
